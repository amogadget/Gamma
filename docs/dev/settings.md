...
| Per workspace | `workspaces` / `workspace_members` in `users.db`, via `/api/workspaces*` ([workspaces.md](workspaces.md)) | name, members and roles, access (private / public + the public role) and a shared workspace's own quota (admins), which workspace this tab works in (`?ws=` in the URL, `gamma-last-ws:<user>` remembers the last one) |
...
- **Workspace** — Members & sharing (this workspace: rename, storage, the
  Access rows for admins, export/import, the member list with role menus,
  Invite through an `AccountPicker` over the account directory, leave,
  delete; all my workspaces with Open; New workspace —
...
- **Account** — Users (admin account management / "You" for non-admins, in
  ...
  access, quota, members and ownership, delete, join — and New workspace for
  any owner, private or public, in
...
inherit) in the Users pane. They apply to the account's personal workspace
only; a shared workspace has its own optional quota (admins, Settings →
