...
Before tagging: run backend tests/Ruff, frontend tests/ESLint/build, Desktop lint/unit tests and validate the extension manifest/package. Inspect the exact staged file list and commit tree. Exclude unfinished clients or local design documents explicitly; never use a blanket `git add .` for a partial release. The native iPad client was excluded from the 0.3.0 coordinated release. Starting with 0.4.0, include the iPad source, tests and self-signing deployment documentation in the source archive. Do not attach personal-team IPAs, certificates, provisioning profiles or local signing overrides as public installers; the iPad source currently shares the Web release commit rather than a separate binary-release tag.
