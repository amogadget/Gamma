...
| 2.7 | Offline copies: on a remote server a workspace row shows *keep offline* on hover; choosing it makes the copy on the local server and moves the window there; the copy's row reads *offline copy* with an *original* chip back to the remote; the remote row now shows *offline copy* and opens the copy; a second *keep offline* opens the existing copy instead of making another | auto: *offline copy: "keep offline" on a remote row makes the copy; the rows then cross-link* |
| 2.8 | At launch the local servers that hold offline copies start in the background (their dots turn on) even when a remote server is the one open | manual |
