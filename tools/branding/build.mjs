...
if (check) {
  ...
  for (const name of ['connections', 'workspaces', 'library']) {
    const script = `tools/branding/build-${name}.py`;
    const result = spawnSync(process.env.PYTHON || 'python', [script], { cwd: ROOT, encoding: 'utf8' });
    if (result.error || result.status !== 0) throw new Error(result.error?.message || result.stderr || result.stdout);
    for (const theme of ['light', 'dark']) {
      const p = `docs/assets/branding/gamma-${name}-${theme}.svg`;
      emit(p, read(p), { source: script, variant: theme });
    }
  }
