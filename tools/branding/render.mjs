...
try {
  ...
  const stems = fs.readdirSync(path.join(ROOT, 'docs/assets/branding')).filter(f => /^gamma-.+-light\.svg$/.test(f)).map(f => f.replace(/\.svg$/, ''));
  for (const stem of stems.filter(stem => !only.length || only.some(name => stem === `gamma-${name}-light`))) {
    {
