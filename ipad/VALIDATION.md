...
## 0.4.0 merge/release preparation

The unified design is `../docs/design/handwriting-recording-ipad.md`; it separates shipped behavior from historical proposals. Before merging the accumulated implementation: backend **489 passed** with Ruff clean; Web **56 passed**, ESLint/build passed; Desktop **16 unit tests passed** and lint passed; Connector 0.1.2 manifest references and ZIP integrity validated. `DesignMerge-1.xcresult` ran the complete native suite: **84 passed, 4 skipped, zero failures**. Skips are two opt-in live-backend tests, the opt-in deployment smoke and the explicitly supplied local preparation-input tool; earlier entries record the live tests that were run separately.

Public distribution plan is Web 0.4.0 / Desktop 0.1.7 / Connector 0.1.2 from one source commit, plus iPad source/self-signing documentation. Actual GitHub workflow/asset completion must be verified after tagging; these pre-release checks alone do not claim installers are published. No personal signing configuration, credentials or private preparation input is part of the release.

## Browser Note Replay delivery

Web now plays AAC against recording events and native-exported, final-surviving stroke PNG/path assets. The player supports progressive reveal, cross-page seek, ink-click seek/resume and clean exit. Missing/stale previews are explicitly gated with optional static fallback. New iPad ink uploads include `.inkjson`; old ink is backfilled through a source-checked metadata-only endpoint, preserving original bytes, comments and ink revisions. See `../docs/dev/note-replay.md`.

Verified: backend full suite **489 passed**; Web unit suite **54 passed**, lint/build passed. Actual Chrome with AAC support passed the complete App audio-block entry/player/PDF/Done flow and the focused progressive-mask, rotated/cropped page, segment-clock and ink-click seek/resume test. The PNG/JSON/AAC fixtures were generated by PencilKit/AVFAudio on the Mac, not substituted images or a fake audio clock. The stock Linux Chromium runtime lacked AAC; an isolated Chrome binary was used instead. `BrowserReplayLive-1.xcresult`: **2 passed**, covering native outbox upload, automatic old-ink backfill, source-byte preservation, cache reopen and audio timeline sync against a disposable real Gamma backend.

Deployed after backup `/home/ubuntu/gamma-deployment-backups/before-browser-replay-20260910T174820Z.tar.gz`; rollback `gamma:before-browser-note-replay`. Public player bundle, replay format, backfill OpenAPI endpoint and health verified. The final deployed frontend also passed the complete Chrome Replay UI smoke test with isolated synthetic API responses (no private account data or production writes). Signed iPad exporter build passed, installed and launched on the connected iPad (sequence 1804). Existing documents need opening/syncing once on updated iPad, then Web refresh. No timestamp-free recording is retroactively synchronized. Long-session memory/smoothness and hardware touch remain acceptance work; in-progress raster reveal is approximate at stroke overlaps. No new release/push was performed for this iteration.

## Full Web workspace reuse and native text highlights

The actual Gamma Web frontend is embedded in an isolated WKWebView after sign-in, sharing the account session without password persistence. Pencil & Audio flushes Web saves and hands off validated page/document identity to native; Full Gamma drains native pending changes then reloads the existing Web store. Hidden Web autosave/unload writes are suppressed during the native session, guarding against stale subtree replacement. Downloads/blob exports route through WKDownload and Share/Save to Files; main-frame-only script dialogs and bridge validation constrain native access. See `WEB_PARITY.md` for the feature matrix and unverified device-specific flows.

Native Select Text uses PDFKit selections, a color toolbar and one ordinary Gamma highlight block per selected page, with an idempotent `/api/blocks/{id}/highlight` endpoint and durable outbox.

Verification: backend native-highlight/block suite 11 passed; frontend tests/lint/build passed (an initial test lint import was corrected). `WebRuntime-1.xcresult` passed 4 tests for actual WebKit bridge handling, origin/payload policy and real PDF selection. `WebCookieBridge-1` and `WebFinal-1` verified cookie revalidation/export and native text selection; final set had 4 passing tests. `WebDeployment-1.xcresult` passed a readonly real-HTTPS deployment smoke test in WKWebView, confirming the Gamma Web page and native bridge load without weakening TLS. It did not authenticate a user's account or invoke paid AI features.

Deployed after backup `/home/ubuntu/gamma-deployment-backups/before-full-web-20260910T153554Z.tar.gz`, rollback image `gamma:before-full-web-workspace`; public health and new highlight route verified. Final Web bundle contains native handoff/save-freeze guards. GUI-session device build/signature verification succeeded; installed and launched on physical iPad, sequence 1796. No push or new release in this iteration. Full touch/provider/import/export parity remains a documented acceptance matrix, not an unsupported claim that Electron's local process/server functionality runs on iPad.
