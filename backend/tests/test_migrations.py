...
def test_v7_adds_mcp_oauth_and_preserves_tokens(data_dir):
    ...
    assert migrations.ensure_current()["applied"] == ["mcp_oauth", "ai_usage", "upload_path_titles", "mirrors", "mirror_cadence"]
...
def test_v6_adds_integration_tokens_and_is_repeatable(data_dir):
    ...
    with closing(sqlite3.connect(str(data_dir / "users.db"))) as conn:
        ...
        conn.commit()
    assert migrations.ensure_current()["applied"] == ["integration_tokens", "mcp_oauth", "ai_usage", "upload_path_titles", "mirrors", "mirror_cadence"]
    with connect_users_db() as conn:
...
def test_v5_adds_publisher_sessions_and_is_repeatable(data_dir):
    ...
    assert result["applied"] == ["publisher_sessions", "integration_tokens", "mcp_oauth", "ai_usage", "upload_path_titles", "mirrors", "mirror_cadence"]
    with connect_users_db() as conn:
...
def test_status_and_refusal_on_a_v0_directory(data_dir):
    ...
    assert [p["name"] for p in st["pending"]] == ["baseline", "workspaces", "workspace_access", "workspace_kinds", "publisher_sessions", "integration_tokens", "mcp_oauth", "ai_usage", "upload_path_titles", "mirrors", "mirror_cadence"]
...
def test_upgrade_v0_to_current(data_dir):
    ...
    assert result["applied"] == ["baseline", "workspaces", "workspace_access", "workspace_kinds", "publisher_sessions", "integration_tokens", "mcp_oauth", "ai_usage", "upload_path_titles", "mirrors", "mirror_cadence"]
...
def test_interrupted_upgrade_resumes(data_dir):
    ...
    assert result["applied"] == ["workspaces", "workspace_access", "workspace_kinds", "publisher_sessions", "integration_tokens", "mcp_oauth", "ai_usage", "upload_path_titles", "mirrors", "mirror_cadence"] and migrations.data_version() == SCHEMA_VERSION
    with connect_users_db() as conn:
...
def test_v9_repairs_leaked_upload_paths_once(data_dir):
    ...
    assert migrations.ensure_current()["applied"] == ["upload_path_titles", "mirrors", "mirror_cadence"]
