...
def _welcome_blocks():
    ...
    return [
        ...
        (
            ...
            wid,
            "a0",
            "Gamma is a self-hosted, Logseq-inspired PDF annotation tool. You can highlight PDFs, organize notes as nested outliner blocks, and share read-only annotated copies via link.",
            "{}",
        ),
        ...
        (
            ...
            started_id,
            "a0",
            "**Open a PDF**: paste a URL in the topbar and click Open, or drag a PDF file onto this page.",
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            started_id,
            generate_key_between("a0", None),
            "**Highlight text**: select text in the PDF to create a highlight with optional comment and color.",
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            started_id,
            generate_key_between("a0V", None),
            "**Add notes**: type in any block. Press Enter for a new sibling, Tab to indent, Shift+Tab to outdent.",
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            started_id,
            generate_key_between("a1", None),
            "**Reorder blocks**: hover over a block's left edge, grab the ⋮⋮ handle, and drag to reorder.",
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            started_id,
            generate_key_between("a1V", None),
            "**Drag images**: drag an image file from your computer onto any block to insert it. You can also paste images from the clipboard.",
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            started_id,
            generate_key_between("a2", None),
            '**AI chat**: click "Show AI Chat" at the bottom of the sidebar to ask questions about the open PDF.',
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            started_id,
            generate_key_between("a2V", None),
            '**Share**: click "Share link" in the ⋮ menu to generate a public read-only link for any annotated PDF.',
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            started_id,
            generate_key_between("a3", None),
            "**Category tags**: add a `category::` tag below the summary to organize pages. The home page groups them into carousels.",
            "{}",
        ),
        ...
        (
            ...
            figures_id,
            "a0",
            "Drag any image file into a block to embed it. Gamma uploads it and inserts `![]()` markdown. Here is what the app looks like:",
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            figures_id,
            generate_key_between("a0", None),
            f"![]({_SCREENSHOTS}/01-annotated-pdf.png)",
            "{}",
        ),
        (
            secrets.token_urlsafe(9),
            figures_id,
            generate_key_between("a0V", None),
            f"![]({_SCREENSHOTS}/02-home-carousels.png)",
            "{}",
        ),
        ...
        (
            ...
            guest_id,
            "a0",
            "You are logged in as a **guest**. Your data resets each day at midnight UTC. To keep your work permanently, ask the admin to create an account for you.",
            "{}",
        ),
        ...
        (
            ...
            md_id,
            "a0",
            "Blocks support **bold**, *italic*, `code`, [links](https://example.com), and inline $\\KaTeX$ math like $E = mc^2$.",
            "{}",
        ),
    ]
    ...
    with connect_users_db() as conn:
        if conn.execute("SELECT 1 FROM users WHERE is_guest = 0").fetchone():
            if not conn.execute("SELECT 1 FROM users WHERE is_admin = 1 AND is_guest = 0").fetchone():
                log.info(
                    "[startup] no account has the admin privilege - grant one with: "
